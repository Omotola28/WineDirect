...
//     
    ...
    public void selectWineCase(WineCase wineCase)
    {
        if(isBuyer == true)
        {
         //wineCase = wineCase1;
         basket.add(wineCase);
         //System.out.println("Buyer "+getID()+" has selected wine case ref number "+wineCase.getRefNo()+" of "+
         //"/n"+wineCase.getNoOfBottles()+ " " +wineCase.getDes()+" at £"+wineCase.getPrice()+"");
        }
        else
        {
    ...
    public void wineCaseInBasket (int index)
    {
        ...
        if(index>=0 && index < basket.size())
        {
            ...
            System.out.println(wineCase.toString());
        }
    }
    ...
    /**
     * Removes unwanted items from basket based on the items index position
     */
     ...
     */
    public void putWineCaseBack(String searchRef)
    {
        int index = 0;
        boolean found = false;
        ...
        if(!found){
            ...
            System.out.println("No Match has been found, basket has not been updated"+
            "\n###########################################################");
        }
        else{
            //found show updated basket
            showBasket();
        }
    }
     ...
     * Willing to spend. If over budget item/items are removed
     */
